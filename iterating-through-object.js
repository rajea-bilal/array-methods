...
function outed(meet, boss){
    ...
    for(let item in meet) {
      if(item === boss){
        totalHappiness = totalHappiness + meet[item] * 2
      } else {
        totalHappiness = totalHappiness + meet[item]
      }
    }
     
    let numOfPeople = Object.entries(meet).length
    // return numOfPeople
      
     let result = totalHappiness / numOfPeople
    // return result
